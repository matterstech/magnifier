	...
	public void generateHtml(String reportFilePath) {
		...
		try {
			...
			html = html + "<html>"
					...
					+ "  </head>"
					+ "  <body>"
					+ "    <h1>Magnifier Report on \"" + databaseName + "\" - " + dateFormat.format(startTime) + " - Execution time " + (days == 0 ? "" : (days+"d")) + (hours == 0 ? "" : (hours+"h")) + ((minutes == 0 ? "" : (minutes+"m"))) + seconds+"s" + "</h1>"
					+ "    <table class=\"rule-table\">"
					+ "      <thead>"
					+ "        <tr>"
					+ "        <th></th>"
					+ "        <th title=\"The percentage of compliant entities\">Rule compliance (debt)</th>"
					+ "        <th>Rule</th>"
					+ "        <th>Suggestions</th>"
					+ "      </tr>"
					+ "    </thead>"
			...
			for(RuleReport rr : ruleReports) {
				...
				html = html + "  <td>"
						...
						+ "    (<span class=\"debt\" title=\"" + rr.getDebt() + " hours to correct (< " + (new Float(rr.getDebt() / 7).intValue() + 1) + " days)\">" + (rr.getDebt() != 0.0 ? rr.getDebt() + "h" : "") + "</span>)"
						+ "  </td>"
